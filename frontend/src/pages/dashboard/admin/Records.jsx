...
const Record = () => {
  ...
  const fetchAlumniData = async () => {
    try {
      const response = await fetch("http://localhost:3000/alumni/list");
      if (!response.ok) {
  ...
  const handleAddFormSubmit = async (e) => {
    e.preventDefault();
    try {
      // Replace the placeholder URL with the actual add alumni endpoint
      const addAlumniEndpoint = "http://localhost:3000/alumni/create";
